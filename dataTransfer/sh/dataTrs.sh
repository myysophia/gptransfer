...
su - gpadmin <<EOF
psql -h 10.50.10.163 -c "copy (select * from $DB_NAME.$SCHEMA_NAME.$TABLE_NAME where evt_timestamp >= '$EXPORT_START_DATE' and evt_timestamp <= '$EXPORT_END_DATE') to PROGRAM 'gzip > $path$DB_NAME^$SCHEMA_NAME^
$TABLE_NAME^$PATH_T.csv.gz'"
# |tee $current_path/result
#psql -t -h 10.50.10.163 -c "select count(1) from $DB_NAME.$SCHEMA_NAME.$TABLE_NAME where evt_timestamp >= '$EXPORT_START_DATE' and evt_timestamp <= '$EXPORT_END_DATE'" > $current_path/$DB_NAME.$SCHEMA_NAME.$TA
BLE_NAME
EOF
#result=`psql -c "copy (select * from $DB_NAME.$SCHEMA_NAME.$TABLE_NAME where evt_timestamp >= '$EXPORT_START_DATE' and evt_timestamp <= '$EXPORT_END_DATE') to PROGRAM 'gzip > /mnt/dataTransfer/wpp_adefect_f_n_
10201650.csv.gz'"|awk '{print $2}'`
# 获取状态码
t=$?

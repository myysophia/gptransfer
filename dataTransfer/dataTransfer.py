...
def connectGP():
    # # try:
    #     global conn
    #     conn = psycopg2.connect(database="", user="", password="", host="10.50.10.163", port="5432")
...
def copyGzip(jobName):
    ...
    try:
        ...
        ssh.connect('10.50.10.', 22, '', '')  # 远程主机IP、端口、用户名、密码
        ...
        if jobInfoList[0]["run_flg"] == 'N':
            ...
            for index in range(len(tableList)):
                    command = 'sh /home/scripts/dumpFile/dataTrs_0204.sh ' \
                       + " '" + str(start) + "' '" \
                              + str(end)+ "' '" \
